...
-- FINAL COMBINED VERSION - EXECUTOR COMPATIBLExx
...
table.insert(connections, RunService.Heartbeat:Connect(function()
    ...
    -- Debug mode: Run comprehensive debugging every 10 seconds (reduced from 3)
    if isTreeDebugMode and currentTime - lastTreeDebugTime > 10 then
        debugTreesAroundPlayer()
        lastTreeDebugTime = currentTime
    end
        
        -- BONUS: List all RemoteEvents in the game for manual analysis
        print("\n--- AVAILABLE REMOTEEVENTS (First 10) ---")
        local remoteCount = 0
        for _, remote in ipairs(game:GetDescendants()) do
            if remote:IsA("RemoteEvent") then
                remoteCount = remoteCount + 1
                if remoteCount <= 10 then -- Reduced from 20 to reduce spam
                    print(string.format("RemoteEvent: %s (Parent: %s)", remote.Name, remote.Parent and remote.Parent.Name or "None"))
                end
            end
        end
        if remoteCount > 10 then
            print(string.format("... and %d more RemoteEvents", remoteCount - 10))
        end
        print(string.format("Total RemoteEvents found: %d", remoteCount))
        
        treeLastDebugPrint = currentTime
    end
    
    -- Get survival tool
    local tool = getSurvivalTool()
    if not tool and player:FindFirstChild("Backpack") then
        -- Try to equip a survival tool from backpack
        for _, backpackTool in pairs(player.Backpack:GetChildren()) do
            if backpackTool:IsA("Tool") then
                local toolName = string.lower(backpackTool.Name)
                if string.find(toolName, "axe") or string.find(toolName, "hatchet") or 
                   string.find(toolName, "pickaxe") or string.find(toolName, "shovel") or
                   string.find(toolName, "hammer") or string.find(toolName, "tool") then
    ...
    -- Find resource targets with ENHANCED detection for survival games
    local allResources = {}
    
    -- Method 1: Search using specific resource folder paths first
    local pathResources = findResourcesInPaths()
    for _, resource in pairs(pathResources) do
        ...
        if targetPart then
            ...
            if distance <= currentTreeAuraRadius then
    ...
    -- Method 2: AGGRESSIVE workspace search - check ALL objects
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj ~= player.Character and obj.Name ~= "Camera" then
            ...
            if targetPart and targetPart:IsA("BasePart") then
                ...
                if success and distance and distance <= currentTreeAuraRadius then
                    -- Check if it's a tree using multiple methods
                    local isTreeTarget = false
                    local objName = string.lower(obj.Name)
                    
                    -- Method A: Check against known tree names
                    for _, treeName in ipairs(TREE_NAMES) do
                    ...
                    -- Method B: Check for common tree/wood keywords
                    if not isTreeTarget then
                        local treeKeywords = {"wood", "log", "trunk", "bark", "oak", "pine", "birch", "maple", "cedar"}
                    ...
                    -- Method C: Check if it starts with common tree prefixes or patterns
                    if not isTreeTarget then
                        if string.match(objName, "^tree") or string.match(objName, "^fence") or string.match(objName, "model%d+") then
                            isTreeTarget = true
                        end
                    end
                    ...
                    if isTreeTarget then
                        ...
                        learnTreePattern(obj)
                        
                        -- Check if not already added
                        local alreadyAdded = false
                        for _, existingTree in pairs(allTrees) do
                            if existingTree.model == obj then
                                alreadyAdded = true
                                break
                            end
                        end
    ...
    -- Method 3: Search ALL folders recursively
    local function searchFolder(folder, folderName)
        for _, obj in ipairs(folder:GetChildren()) do
            if obj:IsA("Folder") then
                searchFolder(obj, folderName .. "/" .. obj.Name)
            elseif obj:IsA("Model") or obj:IsA("BasePart") then
                ...
                if targetPart and targetPart:IsA("BasePart") then
                    ...
                    if success and distance and distance <= currentTreeAuraRadius then
                        ...
                        -- More aggressive tree detection
                        for _, treeName in ipairs(TREE_NAMES) do
                            if string.find(objName, string.lower(treeName)) then
                                isTreeTarget = true
                                break
                        ...
                        -- Also check folder names for tree context
                        if not isTreeTarget then
                            local folderNameLower = string.lower(folderName)
                            if string.find(folderNameLower, "tree") or string.find(folderNameLower, "wood") or string.find(folderNameLower, "forest") then
                                isTreeTarget = true
                            end
                        end
