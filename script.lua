...
--[[ ... Other GUI Elements ... ]]
-- (The code for labels, toggles, and sliders is unchanged)
...
closeButton.Position = UDim2.new(1, -25, 0, 5) -- Top right corner
...
local function updateToggleVisuals(button, enabled)
    ...
    button.BackgroundColor3 = enabled and Color3.fromRGB(80, 180, 80) or Color3.fromRGB(180, 80, 80)
end

table.insert(connections, infJumpToggle.MouseButton1Click:Connect(function()
    isInfJumpEnabled = not isInfJumpEnabled
    updateToggleVisuals(infJumpToggle, isInfJumpEnabled)
end))

table.insert(connections, auraToggle.MouseButton1Click:Connect(function()
    isAuraEnabled = not isAuraEnabled
    updateToggleVisuals(auraToggle, isAuraEnabled)
end))

-- Self-destruct logic for the close button
table.insert(connections, closeButton.MouseButton1Click:Connect(function()
    -- Reset any changes the script made
    if humanoid then
        humanoid.WalkSpeed = WALK_SPEED_DEFAULT
    end

    -- Disconnect all events to stop the script from running
...
local function createSliderLogic(handle, track, minVal, maxVal, label, updateCallback)
    ...
    table.insert(connections, handle.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then isDragging=true end end))
    table.insert(connections, handle.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then isDragging=false end end))
...
createSliderLogic(radiusSliderHandle, radiusSliderTrack, AURA_RADIUS_MIN, AURA_RADIUS_MAX, radiusLabel, function(value)
    currentAuraRadius = value; radiusLabel.Text = "Aura Radius: " .. value
end)
createSliderLogic(speedSliderHandle, speedSliderTrack, WALK_SPEED_MIN, WALK_SPEED_MAX, speedLabel, function(value)
    currentWalkSpeed = value; speedLabel.Text = "Walk Speed: " .. value
    if humanoid then humanoid.WalkSpeed = value end
end)

local isDraggingWindow, dragStart, startPos = false
table.insert(connections, titleLabel.InputBegan:Connect(function(input)
    if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then isDraggingWindow,dragStart,startPos=true,input.Position,mainFrame.Position end end))
table.insert(connections, titleLabel.InputEnded:Connect(function(input)
    if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then isDraggingWindow=false end end))
table.insert(connections, UserInputService.InputChanged:Connect(function(input)
...
table.insert(connections, RunService.Heartbeat:Connect(function()
    if not isAuraEnabled or not player.Character or not player.Character.PrimaryPart then return end
    ...
    if not damageEvent or not damageEvent:IsA("RemoteEvent") then return end

    local playerPos = player.Character.PrimaryPart.Position
    for _, target in ipairs(workspace:GetDescendants()) do
        if target:IsA("Humanoid") and target.Health > 0 then
            local targetChar = target.Parent
            if targetChar ~= player.Character and targetChar:FindFirstChild("HumanoidRootPart") then
                local dist = (playerPos - targetChar.HumanoidRootPart.Position).Magnitude
                if dist <= currentAuraRadius then
                    if not auraTargetDebounce[target] or tick() - auraTargetDebounce[target] > 0.2 then
                        auraTargetDebounce[target] = tick()
